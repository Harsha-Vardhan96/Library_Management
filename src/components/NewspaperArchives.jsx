...
const NewspaperArchives = ({ toggleTheme, isDarkMode, toggleFullScreen }) => {
    ...
    const sections = [
        {
            ...
            items: [
                ...
                'Indian Express', 'The Economic Times', 'The Statesman',
                'Deccan Herald', 'The Telegraph', 'Business Standard',
                'The Pioneer', 'Financial Express', 'Asian Age'
            ]
        },
        {
    ...
    return (
        <div className="flex flex-col w-full min-h-screen bg-[#f8f9fa] dark:bg-ndl-dark transition-colors duration-500">
            {/* Top Utility Bar */}
            <div className="z-[1050] bg-gradient-to-r from-[#455a64] to-[#263238] text-white flex justify-between px-[5%] py-2.5 text-xs font-semibold tracking-wide sticky top-0 border-b border-white/5 backdrop-blur-sm shadow-lg">
                <div className="flex items-center gap-6">
                    ...
                    <button className="flex items-center gap-2 opacity-90 hover:opacity-100 hover:text-accent transition-all group p-1">
                        ...
                        <span className="hidden sm:inline">News Terminal</span>
                    </button>
                </div>
                <div className="flex items-center gap-6">
                    <button
            ...
            {/* Hero Header */}
            <div className="relative pt-20 pb-12 px-[5%] text-center overflow-hidden bg-gradient-to-b from-[#eceff1] to-transparent dark:from-ndl-dark dark:to-transparent">
                <div className="relative z-10 flex flex-col items-center">
                    ...
                    <div className={`inline-block px-14 py-4 ${isDarkMode ? 'bg-gray-800 border-white/10' : 'bg-white border-white'} shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-3xl mb-10 border animate-popIn`}>
                        <h1 className={`text-4xl font-black tracking-tighter uppercase ${isDarkMode ? 'text-white' : 'text-ndl-dark'}`}>
                            Newspaper <span className="text-accent underline decoration-primary/20 transition-all">Archives</span>
                        </h1>
                    </div>
                    ...
                    <div className="w-full max-w-5xl relative group mb-12 animate-fadeInUp">
                        ...
                        <div className="relative bg-white dark:bg-gray-800 border-2 border-transparent focus-within:border-accent shadow-2xl rounded-[1.5rem] flex p-1 transition-all duration-500 overflow-hidden">
                            <div className="px-8 flex items-center border-r border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-gray-900/20">
                                <span className="text-xs font-black uppercase tracking-widest text-[#455a64] dark:text-[#90a4ae] flex items-center gap-3">
                                    🗞️ Press Hub
                                </span>
                            </div>
                            <input
                                type="text"
                                placeholder="Search headlines, dates, publications, or historical reports..."
                                value={searchTerm}
                            ...
                            <button
                                ...
                                className="bg-[#455a64] hover:bg-[#263238] text-white px-12 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-xl active:scale-95 shadow-primary/30"
                            >
                                Search
                            </button>
                        </div>
                        <div className={`mt-4 flex justify-center gap-6 text-[10px] font-black uppercase tracking-widest ${isDarkMode ? 'text-gray-400' : 'text-text-gray/80'}`}>
                            <span
                                onClick={() => alert('Fetching latest headlines from today\'s editions...')}
                            ...
                            <span
                                onClick={() => alert('Accessing historical Gazette archives...')}
                                className="cursor-pointer hover:text-accent transition-colors"
                            >
                                Historical Gazettes
                            </span>
                            <span className="w-1 h-1 bg-gray-300 rounded-full my-auto"></span>
                            <span
                                onClick={() => alert('Browsing regional press terminal...')}
                                className="cursor-pointer hover:text-accent transition-colors"
                            >
                                Regional Press
                            </span>
                    ...
                    {sections.map((section, idx) => (
                        ...
                        >
                            ...
                            <div className="p-12 flex-1 relative overflow-hidden">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6 relative z-10">
                                    {section.items.map((item, itemIdx) => (
                                        <div
                                            key={itemIdx}
                                            onClick={() => alert(`Opening publication: ${item}`)}
                            ...
                            <div className="px-12 py-6 bg-gray-50/50 dark:bg-black/30 border-t border-gray-100 dark:border-white/5 flex justify-center group-hover:bg-accent/10 transition-colors duration-500">
                                <button
                                    onClick={() => alert(`Loading digital archives for ${section.title}...`)}
                                    className="flex items-center gap-4 text-xs font-black uppercase tracking-[0.4em] text-[#455a64] dark:text-[#90a4ae] hover:text-[#263238] transition-all group/btn"
                                >
            ...
            <footer className="py-20 px-[5%] bg-white dark:bg-ndl-dark border-t border-border-color dark:border-white/10 mt-auto">
                <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row justify-between items-center gap-12">
                    ...
                    <div className="text-text-gray/50 dark:text-gray-500 text-[10px] font-black tracking-[0.4em] uppercase text-center lg:text-right">
                        Documenting the Present for the Future &copy; 2026 NDLI Platform
                    </div>
                </div>
            </footer>
        </div>
...
export default NewspaperArchives;
