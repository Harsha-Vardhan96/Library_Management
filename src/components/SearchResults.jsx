...
const SearchResults = () => {
    ...
    return (
        <div className="flex flex-col w-full min-h-screen bg-ndl-dark transition-colors duration-500">
            {/* Top Utility Bar */}
            ...
            <div className="flex flex-1 max-w-[1600px] mx-auto w-full px-[5%] py-12 gap-12">
                ...
                <aside className="w-72 hidden lg:flex flex-col gap-8 shrink-0">
                    <div className="bg-gray-800 p-8 rounded-3xl shadow-sm border border-white/5">
                            ...
                            <div>
                                ...
                                {['Book', 'Article', 'Thesis', 'Video', 'Dataset'].map(type => (
                                    <label key={type} className="flex items-center gap-3 mb-3 cursor-pointer group">
                                        ...
                                        <span className="text-sm font-medium text-white/80 group-hover:text-accent transition-colors">{type}</span>
                                    </label>
                                ))}
                            </div>
                            ...
                            <div>
                                ...
                                {['IIT Madras', 'Supreme Court', 'NCERT', 'NPTEL', 'UGC'].map(org => (
                                    <label key={org} className="flex items-center gap-3 mb-3 cursor-pointer group">
                                        <input type="checkbox" className="w-4 h-4 rounded border-border-color text-accent focus:ring-0" />
                                        <span className="text-sm font-medium text-white/80 group-hover:text-accent transition-colors">{org}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                ...
                <main className="flex-1 min-w-0">
                    ...
                    <div className="mb-12">
                        ...
                        <div className="bg-gray-800 p-2 shadow-2xl rounded-2xl flex border border-white/5">
                            <input
                                type="text"
                                value={searchTerm}
                                ...
                                className="flex-1 bg-transparent px-8 py-4 text-lg outline-none text-white font-medium"
                            />
                            <button
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 gap-6 animate-fadeInUp">
                            {results.map((res) => (
                                <div key={res.id} className="group bg-gray-800 p-8 rounded-3xl border border-transparent hover:border-accent/20 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                                    <div className="flex gap-6 items-center">
                                        <div className="w-16 h-16 bg-black/20 rounded-2xl flex items-center justify-center text-3xl shadow-inner group-hover:scale-110 group-hover:rotate-6 transition-transform">
                                        ...
                                        <div>
                                            ...
                                            <h3 className="text-xl font-black text-white tracking-tight mb-1 group-hover:text-primary transition-colors">{res.title}</h3>
                                    ...
                                    <div className="flex gap-3 w-full md:w-auto">
                                        <button
                                            ...
                                            className="flex-1 md:flex-none px-6 py-3 bg-black/20 text-gray-300 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-primary hover:text-white transition-all active:scale-95 border border-white/5"
                                        >
                                            View Online
                                        </button>
                        ...
                        <div className="text-center py-32 bg-gray-800/50 rounded-[3rem] border-2 border-dashed border-white/5 animate-popIn">
                            <div className="text-6xl mb-6 opacity-30">🔍</div>
                            <h2 className="text-2xl font-black text-white mb-4 tracking-tight">No resources matched your search</h2>
            ...
            <footer className="py-12 px-[5%] bg-ndl-dark border-t border-white/5 mt-auto">
